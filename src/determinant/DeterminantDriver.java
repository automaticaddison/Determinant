            	...
            	if (x.length == (n * n)) {
            		...
            		for (int a = 0; a < m.length; a++) {
            			for (int b = 0; b < m[a].length; b++) {
            				m[a][b] = Integer.parseInt(x[counter]);
            				counter++;
            			}
            		}
            	}
...
class Determinant {
	 ...
	 */
	public double det(int[][] matrix) {
		int matrixOrder = matrix.length;
		
		if (matrixOrder == 1) {
			return matrix[0][0]; // Stopping case
		}
			...
			for (int b = 0; b < matrixOrder; b++) {				
				sum += Math.pow(-1, b) * matrix[0][b] * 
						det(minor(matrix, 0, b));  // Recursive call
			}
			return sum;			
		}
	 ...
	 */
	private int[][] minor(int[][] matrix, int i, int j) {
		
		int matrixOrder = matrix.length;
		int minorOrder = matrixOrder - 1; // The order of the submatrix
		int[][] minor = new int[minorOrder][minorOrder];
		int a = 0;
		int b = 0;
